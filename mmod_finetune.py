...
def build_model(model_opt, opt, fields, checkpoint):
    ...
    for name, param in model.named_parameters():
        if name.startswith('generator') and mmod_generator \
                         and not opt.mmod_freeze_generator:
    ...
    if mmod_generator:
        ...
        model.generator = onmt.modules.multimodal.MultiModalGenerator(
            ...
            add=opt.mmod_generator_add, use_hidden=opt.mmod_use_hidden)
    if len(opt.gpuid) > 1:
...
def main():
    ...
    train_img_mask = train_img_mask.astype(np.byte)
    valid_img_mask = valid_img_mask.astype(np.byte)
